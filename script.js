document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateField(field) {
    ...
    if (trimmedValue && type === 'email' && !validateEmail(trimmedValue)) {
      showFieldError(field, getValidationMessage('email'));
      return false;
    }

    if (trimmedValue && type === 'tel' && !validatePhone(trimmedValue)) {
      showFieldError(field, getValidationMessage('phone'));
      return false;
    }

    if (trimmedValue && minLength && trimmedValue.length < parseInt(minLength, 10)) {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    if (submitBtn) {
      ...
      submitBtn.textContent = isRTL ? 'جاري الإرسال...' : 'Sending...';
    }
  ...
  function initScrollAnimations() {
    ...
    elements.forEach(function (el, index) {
      ...
      const delay = Math.min(index % 4 * 100, 300);
      if (delay > 0) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Global Music Pro משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '082a1dc9-2014-4abf-8169-f78e798c94a1',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '082a1dc9-2014-4abf-8169-f78e798c94a1';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '082a1dc9-2014-4abf-8169-f78e798c94a1';
